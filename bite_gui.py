from bite_email import DailyBitesEmail
from bite_schedule import DailyBitesScheduler
from tkinter import *
...
class DailyBitesGUI:
    ...
    def __init__(self, root):
        #build GUI 
        self.__root = root
        self.__root.title('Daily Bites')
        title_label = ttk.Label(self.__root, text = ' **** Daily Bites ****',
                                font = 'Castellar 32 bold', justify = CENTER)
        ...
        self.__news_var = IntVar()
        self.__weather_var = IntVar()
        self.__recipes_var = IntVar()
        ...
        self.__email = DailyBitesEmail()

        self.__add_recipient_var.set('')
        self.__recipient_list_var.set(self.__email.recipients)
        
        #default send time
        self.__hour_var.set('08') 
        self.__minute_var.set('00')
        
        self.__news_var.set(self.__email.content['news']['include'])
        self.__weather_var.set(self.__email.content['weather']['include'])
        self.__recipes_var.set(self.__email.content['recipes']['include'])
        

        self.__sender_email_var.set(self.__email.credentials['email'])
        self.__sender_password_var.set(self.__email.credentials['password'])

        # initialize scheduler
        self.__scheduler = DailyBitesScheduler()
    ...
    def __build_gui_schedule(self, master, hour_var, minute_var):
        # create GUI widgets
        header = ttk.Label(master, text = 'Scheduled Time (24h):', style = 'Header.TLabel')
    ...
    def __build_gui_contents(self, master, news_var, weather_var, recipes_var):
        # create GUI widgets
        header = ttk.Label(master, text = 'Bite Contents:', style = 'Header.TLabel')
        news_checkbox = Checkbutton(master, text = 'Fox News Headlines',
                                     onvalue = True, offvalue = False, 
                                     variable = news_var)
        weather_checkbox = Checkbutton(master, text = 'Weather Forecast',
                                          onvalue = True, offvalue = False, 
                                          variable = weather_var)
        recipes_checkbox = Checkbutton(master, text = 'Recipes',
                                           onvalue = True, offvalue = False,
                                           variable = recipes_var)
       
        # place GUI widgets using grid geometry manager
        header.grid(row = 0, column = 0, columnspan=2)
        news_checkbox.grid(row = 1, column = 0, sticky = W)
        weather_checkbox.grid(row = 2, column = 0, sticky = W)
        recipes_checkbox.grid(row = 1, column = 1, sticky = W)

    
    #gui elements for sender credentials config
    ...
    # gui elements to update settings & manually send Bites email
    ...
    # Callback function to remove selected recipients
    ...
    def __update_settings(self):
        ...
        self.__email.content['news']['include'] = self.__news_var.get()
        self.__email.content['weather']['include'] = self.__weather_var.get()
        self.__email.content['recipes']['include'] = self.__recipes_var.get()

        self.__email.credentials = {'email': self.__sender_email_var.get(),
    ...
    # Callback function to manually send Bites email
     
    def __manual_send(self):
        # note: settings are not updated before manual send
        print('Manually sending email Bites...')
        self.__email.send_email()
