...
class DailyBitesScheduler(threading.Thread):
    ...
    #Start the scheduler as a separate thread
...
# test DailyBitesScheduler 
    import bite_email
    email = bite_email.DailyBitesEmail()

    scheduler = DailyBitesScheduler()
    scheduler.start()

    hour = time.localtime().tm_hour
    minute = time.localtime().tm_min + 1 #schedule for the next minute
